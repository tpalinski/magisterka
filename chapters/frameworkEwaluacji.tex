...
Reprezentacja wektorowa & embedding tekstu i audio, fuzja osadzeń, indeksowanie korpusu & modele embeddingowe \\
...
&\;
\text{fuzja osadzeń} &: \text{V}[s_1] \times \dots \times \text{V}[s_n] \rightarrow \text{V}[s'],
\\[3pt]
...
albo fragmenty pełnych artykułów najbliższe pytaniu w przestrzeni osadzeń.
...
wyłącznie część opisową odpowiedzi, ponieważ decyzję ocenia się przez porównanie z etykietą zbioru. Trzy pozostałe metryki opierają się na przecięciu zbiorów:
...
nieobecnych w kontekście, $\mathrm{PK}$ pokrycie kontekstu, a $\mathrm{BD}$ bezpieczeństwo
dawkowania. Symbole $T$, $W$ i $P$ oznaczają odpowiednio zbiory tokenów, słów oraz par złożonych
z liczby i jednostki dawki, a indeksy wskazują, czy pochodzą z odpowiedzi wygenerowanej, z tekstu
...
potrzebne, więc odpowiedź nie mogła zostać poprawnie ugruntowana. Gdy tekst referencyjny nie
...
Trzy ostatnie porównują wyłącznie słowa i liczby, bez oceny sensu
...
dokumentów, więc raportowana wartość MAP jest w praktyce MAP@$k$. W obliczeniach elementowych,
w tym w różnicach parowanych między gałęziami, używa się składników powyższych średnich:
...
identyfikatora producenta i nazwy artefaktu. Odczyt przechodzi listę powiązań w tym porządku
i zwraca wartość pierwszego producenta, który faktycznie coś opublikował.
...
\text{Recall@}k_{\,\text{ASR}}(q) - \text{Recall@}k_{\,\text{referencyjna}}(q).
\]
...
\cite{efron1979bootstrap}. Procedura bootstrapu losuje ze zwracaniem $B$ prób o liczności równej liczbie
par, dla każdej wyznacza średnią różnicę, a granice przedziału odczytuje jako kwantyle rozkładu
tych średnich:

\begin{equation}
\begin{aligned}
\bar{\Delta}^{*(b)} &= \frac{1}{n} \sum_{i=1}^{n} \Delta_{J_i^{(b)}},
\qquad J_i^{(b)} \sim \mathcal{U}\{1, \dots, n\}, \\[4pt]
\mathrm{PU}_{1-\alpha} &= \Big[\, Q_{\alpha/2}\big(\bar{\Delta}^{*}\big),\;
                                    Q_{1-\alpha/2}\big(\bar{\Delta}^{*}\big) \,\Big],
\qquad
...
gdzie $\Delta_i$ oznacza różnicę wartości metryki dla $i$-tego elementu między porównywanymi
gałęziami, $n$ liczbę par, $J^{(b)}$ indeksy wylosowane w $b$-tej próbie, $Q_p$ kwantyl rzędu $p$,
a $\bar{\Delta}$ i $s_{\Delta}$ średnią oraz odchylenie standardowe różnic. Przyjęto
$B = 1000$ oraz $\alpha = 0{,}05$. Ponieważ losowanie odbywa się na samych różnicach, a nie na
wynikach obu gałęzi osobno, przedział zachowuje sparowanie. Rozmiar efektu $d$ jest sparowaną
postacią statystyki Cohena (oznaczaną $d_z$): różnicę średnią odnosi do rozproszenia samych
różnic, nie do rozproszenia wyników w gałęziach. Wartość $p$ pochodzi z testu rang znakowanych
Wilcoxona dla par, który zgodnie ze swoją definicją pomija pary o różnicy zerowej; metryki
elementowe są dyskretne, więc takich par bywa wiele i liczbę par wchodzących do testu raportuje
się osobno, aby wartość $p$ pozostała interpretowalna. Przedział ufności liczony jest natomiast
na pełnym zbiorze par, łącznie z zerowymi, więc estymuje średnią różnicę bez przesunięcia. Przy
wielu metrykach i parach gałęzi stosuje się korekcję Benjaminiego--Hochberga
\cite{benjamini1995fdr}, a obok wartości $p$ podaje rozmiar efektu.
...
Embedding tekstu          & \texttt{labse} \cite{feng-etal-2022-language}, \texttt{jina\_v4} \cite{jina_v4}, \texttt{bge\_m3}, \texttt{clip} \cite{radford2021learning}, \texttt{clap\_text}, \texttt{nemotron}, \texttt{sonar} \\
...
Osobne ograniczenia wnosi materiał badawczy. Ścieżka głosowa nie pochodzi z nagrań, lecz jest
