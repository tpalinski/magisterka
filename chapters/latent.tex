...
Pierwszym z nich był model LaBSE autorstwa Google Research \cite{feng-etal-2022-language}. Jest to model embeddingowy, którego celem jest jednolita reprezentacja danych wejściowych w jednej przestrzeni, bez względu na ich język. Jest on oparty na architekturze transformer inspirowanej modelem BERT i produkuje embeddingi o jednolitej długościdla różnych zdań. Podczas treningu, zdania o tej samej semantyce, lecz w różnych językach, osadzane były w przestrzeni jak najbliżej siebie. W ten sposób model osiąga bardzo dobre wyniki w wyszukiwaniu informacji w różnych językach (metryka accuracy rzędu 95\% na zbiorze Tatoeba \cite{tiedemann-2020-tatoeba}).

Drugim z wykorzystywanych modeli był model Clip autorstwa OpenAI \cite{radford2021clip}. Jest to model multimodalny, pozwalający na osadzanie zarówno tekstu, jak i obrazu, w jednej przestrzeni. Pozwala to na wyszukiwanie m.in obrazu na podstawie tekstu. Przez charakterystykę danych treningowych, czyli obrazy i ich opisy w języku naturalnym, jest stworzony do radzenie sobie z danymi o charakterze opisowym. Jednakże, brak wsparcia dla języków poza językiem angielskim stanowi istotne ograniczenie do części naszych eksperymentów.
...
\begin{figure}
    ...
    \caption{Histogram rozkładu przykładów dla modelu Labse w ramach zbioru \textit{Medical Speech, Transcription, and Intent}. Kolorem pomarańczowym oznaczono ilość zdań, dla których dany przykład jest przykładem negatywnym, niebieskim -- pozytywnym}
...
\begin{figure}
    ...
    \caption{Histogram rozkładu przykładów dla modelu Labse w ramach zbioru FLEURS. Kolorem pomarańczowym oznaczono ilość zdań, dla których dany przykład jest przykładem negatywnym, niebieskim -- pozytywnym}
    \label{fig:latent:labseDistributionFleurs}
\end{figure}

Rozkłady przykładów $x^n$ i $x^p$, dzięki zastosowaniu losowego wyboru były dosyć równomierne dla obu zbiorów danych, co przedstawiono na wykresach \ref{fig:latent:labseDistribution} i \ref{fig:latent:labseDistributionFleurs}.
...
Rozkłady wartości metryki $S(x_{emb,asr})$ \ref{eq:tripletDiff} dla poszczególnych modeli audio przedstawione są na wykresach \ref{fig:latent:medicalData2Vec}, \ref{fig:latent:medicalWav2Vec}, \ref{fig:latent:medicalWhisper} i \ref{fig:latent:medicalHubert}.
...
Tabela \ref{tab:latent:haniaCounts} przedstawia ilość przykładów w ramach zbioru danych \textit{Medical Speech, Transcription, and Intent}, dla których metryka $S(x_{emb,asr})$ \ref{eq:tripletDiff} przyjmowała wartości dodatnie, czyli tam, gdzie model ASR z powodzeniem uchwycił relacje semantyczne. Widać na niej wyraźną przewagę modeli Whisper i Hubert, które uchwyciły podobieństwo dla podobnie wysokiej liczby przykładów w ramach tego zbioru danych.

Tabela \ref{tab:latent:haniaDist} ilustruje z kolei parametry dopasowanego przez nas rozkładu Student-t dla zagregowanych różnic podobieństwa w ramach wszystkich modeli embeddingowych. Liczba stopni swobody $\nu$ odpowiedzialna jest za opisanie stabilności rozkładu w świetle obecności ogonów - im większa, tym bardziej stabilny jest rozkład. Wartości zbliżone do 2, jakie otrzymaliśmy dla modeli Data2Vec, Wav2Vec i Whisper wskazują na ich wysoką niestabilność w uchwyceniu relacji semantycznych. HuBERT okazuje się najbardziej przewidywalnie zachowującym się modelem, dla którego $\nu = 3.6$. Znormalizowana lokalizacja ($\mu/\sigma$) pozwala nam na porównanie separacji pozytywnych i negatywnych przykładów między ewaluowanymi modelami ASR (MMD nie zachowuje skali pomiędzy modelami). Po raz kolejny, model HuBERT wykazuje się najsilniejszą zdolnością rozróżniania przykładów pozytywnych i negatywnych ($\mu/\sigma = 0.61$), wraz z modelem Whisper plasującym się tuż za nim ($\mu/\sigma = 0.54$). Data2Vec i Wav2Vec przejawiają podobnie niską zdolność dyskryminacji ($\mu/\sigma \approx 0.33$). Ostatnią z badanych metryk jest prawdopodobieństwo prawidłowego uchwycenia relacji między $x^a$, $x^n$ i $x^p$. Jest ona podobna do podejścia stricte ilościowego, lecz pozwala ona również analizować osiągi modelu w świetle dopasowanego przez nas rozkładu. Podobnie jak w porównaniu ilościowym, zarówno Whisper jak i HuBERT wykazują najlepsze wyniki. Warto zauważyć jednak, że HuBERT zdecydowanie wysuwa się na prowadzenie w świetle tego porównania, umacniając swoją pozycję jako model najlepiej enkodujący relacje semantyczne w ramach zbioru \textit{Medical Speech, Transcription, and Intent}, dla którego $P(neg > pos) = 71\%$, plasując się przed modelem Whisper, dla którego $P(neg>pos) = 67.8\%$.
% TODO - m4t, mhubert
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Jina} & \textbf{LaBSE} & \textbf{STSB} \\
\midrule
HuBERT   & 3801 & 3971 & \textbf{3827} \\
Wav2Vec  & 3352 & 3377 & 3472 \\
Data2Vec & 3290 & 3337 & 3375 \\
Whisper  & \textbf{3848} & \textbf{4006} & 3802 \\
\bottomrule
\end{tabular}
\end{table}
...
HuBERT    & \textbf{3.6} & \textbf{0.61} & \textbf{71.0\%} \\
\hline
\end{tabular}
...
Wyniki dla dwujęzycznej wersji zbioru Fleurs (język polski i angielski) zostały przedstawione dla modeli ASR wspierających oba z tych języków, reprezentujące trzy rodziny modeli - BeRT (mHuBERT), Whisper i SeamlessM4T. Rozkłady dla tych modeli przedstawione zostały na rysunkach \ref{fig:latent:fleursWhisper}, \ref{fig:latent:fleursMHubert} i \ref{fig:latent:fleursM4T}. Tabele \ref{tab:latent:fleursCounts} i \ref{tab:latent:fleursDist} przedstawiają ilościowe porównanie wyników eksperymentu.
...
Widoczny jest tutaj wyraźnie fenomen stożka, zgodny z zależnością zaobserwowaną w oryginalnej pracy \cite{cone} -- wraz ze wzrostem liczby parametrów i warstw, reprezentacje latentne zajmują coraz to mniejszy fragment przestrzeni (rozkład dla mHuBERT -- 94.4 miliona parametrów [rysunek \ref{fig:latent:coneMHubert}] a ten sam rozkład dla SeamlessM4T -- 2 miliardy parametrów [rysunek \ref{fig:latent:coneM4T}]). Patrząc na te rozkłady w świetle separacji między grupami językowymi. W celu stwierdzenia, czy klastry dla danych w różnych językach należy spojrzeć na odległość między wykresami gęstości dla podobieństw kosinusowych wewnątrz języków i między językami. Modele mHuBERT i SeamlessM4T wydają się enkodować informacje agnostycznie od języka, z separacją między grupami językowymi będącą wyraźnie najmniejszą dla pierwszego modelu (częściowe pokrycie i niewielkie przesunięcie gęstości między grupami a wewnątrz grup na rysunkach \ref{fig:latent:coneMHubert} i \ref{fig:latent:coneM4T}). Model Whisper wykazuje dosyć dużą separację między grupami językowymi w ramach reprezentacji latentnych (rysunek \ref{fig:latent:coneWhisper}). Właściwość ta może prowadzić do mniej dokładnego wyszukiwania informacji między językami polskim i angielskim, a także do gorszego transferu wiedzy pozyskanej w ramach treningu na specjalistycznych zbiorach w języku angielskim do analogicznej wiedzy w języku polskim.
...
Podsumowując, zabieg ABTT zdecydowanie poprawił ogólny rozkład embeddingów w przestrzeni, nie do końca redukując (a dla modelu Whisper pogłębiając) separację między reprezentacjami różnych języków. Wybielanie pozwoliło również na redukcję efektu stożka, tym samym dekorelując embeddingi w świetle języka, potencjalnie poprawiając transfer wiedzy w ramach treningu na danych w języku angielskim.

\subsection{Wnioski}
\subsubsection{Porównanie różnic w semantyczności reprezentacji wektorów cech}
Analiza roznic miedzy modelami embeddingowymi i generalizacji, ktore daja modele ASR miedzy modelami embeddingowymi, omowienie relacji miedzy dopasowanym rozkladem beta a ilosciami przykladow pozytywnych. Generalizacja miedzy zbiorami danych i kernelami.
\subsubsection{Dobór modelu ASR do zintegrowanego systemu przetwarzania mowy i wyszukiwania informacji}
Do uzupełnienia po pełnej analizie wyników eksperymentów
Na podstawie powyzszych wnioskow wybrac jeden z proponowanych modeli - Whisper, ewentualnie ewaluowac dalej HuBert, otrzymalismy rowniez satysfakcjonujące wyniki. Modele nie modelują idealnie semantyki, lecz whisper i hubert radzą sobie dobrze z relacjami (emergent semantic representations).
