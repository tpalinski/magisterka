...
Celem eksperymentów opisanych w tym rozdziale jest zbadanie zdolności różnych modeli ASR, cechujących się różnymi architekturami i reżimami treningowymi, do enkodowania semantyki danych wejściowych. Dodatkowo eceniana była również charakterystyka ich reprezentacji ukrytych dla różnych języków (polskiego i angielskiego) w świetle potencjalnego powstawania klastrów w przestrzeni latentnej im odpowiadającym. Całościową hipotezą weryfikowaną w ramach tego rozdziału jest pytanie, czy wybrane modele cechują się diametralnie różnym stopniem zdolności enkodowania semantycznego danych wejściowych i czy reprezentacje te zależne są od języka wypowiedzi wejściowej. Model, którego przestrzeń ukryta wykazuje cechy tej enkodującej znaczenie danych wejściowych i zachowuje się podobnie dla wypowiedzi w języku polskim i angielskim, stanowiłby potencjalnie najleszego kandydata dla dalszych eksperymentów opisanych w tej pracy.

\subsection{Semantyczne a syntaktyczne reprezentacje audio w wektorach cech}
Dla proponowanego w tej pracy rozwiązania, potencjalnie najważniejszą charakterystyką, którą wykazują się dostępne modele ASR jest postać ukrytej (latentnej) reperezentacji enkodowanej przez nie na podstawie danych wejściowych. Potencjalnie, większa wierność znaczeniu wypowiedzianego zdania doprowadzi do dokładniejszej translacji wyjścia enkodera na współdzielną przestrzeń embeddingów, czego konsekwencją będą lepsze osiągi systemu wyszukiwania dla użytkownika końcowego.
\subsection{Wpływ charakterystyki wektorów cech na jakość embeddingów w modelach wielojęzykowych}
Kolejnym aspektem ecenianym w ramach tego eksperymentu są podobieństwa wyników modeli dla zdań o tym samym znaczeniu w kilku językach. Dla modeli enkodujących semantykę danych, reprezentacje dla dwóch znaczeniowo identycznych zdań w dwóch różnych językach powinny być do siebie podobne według przyjętej metryki podobieństwa. Przykładowo, miara podobieństwa dla reprezentacji latentnych dla zdań \textit{,,Kocham zwiedzać włoskie miasta''} i \textit{,,I love sightseeing Italian cities''}, dla takiego modelu, powinna mieć jak największą wartość.

Należy mieć na uwadze, że różnice w historii języka, jego strukturze i alfabecie istotnie wpływają na otrzymane wyniki podobieństwa dla takich zdań. Zróżnicowanie to zostało zbadane przez Aleshinę i in. w ich pracy ewaluującej rozkłady embeddingów zdań dla dziesięciu różnych języków \cite{multilanguageEmbeddingDifferences}. Korzystając z modelu MUSE \cite{museModel} operującego na pojedynczych słowach, wykazali oni znaczące pokrycie klastrów embeddingów dla języków o podobnej historii (arabski i hiszpański), jak i o podobnie prostej morfologii i częściowo współdzielonym słownictwie (angielski, chiński, indonezyjski), i izolację klastrów dla wybranych języków (rosyjski, o innym alfabecie i bardziej skomplikowanej morfologii w świetle inflekcji języka). Najbardziej interesującym w kontekście tej pracy wnioskiem są podobieństwa dla słów zapożyczonych z innych języków -- wiele terminów medycznych wywodzi się z języka łacińskiego i greckiego, co oznacza potencjalnie wysokie podobieństwo dla takich zwrotów w języku angielskim i polskim, jak i w innych językach \cite{latinMedical}. Dodatkowo, oprócz współdzielenia terminów z greki i łaciny, polskie słownictwo medyczne korzysta z wielu terminów angielskich, co stanowi silny sygnał potwierdzający wysokie pokrycie reprezentacji w ramach domeny medycznej dla obu języków \cite{medicalPolishEnglish}.

Wysoka miara podobieństwa dla modelu ASR ecenianego dla danych wielojęzykowych oznaczałaby nie tylko dobre radzenie sobie z samą semantyką wypowiedzianych zdań, lecz także potencjalnie wysoki potencjał do przeprowadzenia treningu modelu na specjalistycznych danych dostępnych w innych językach. Model wykazujący się taką cechą byłby niezwykle cenny dla poprawienia jakości wyszukiwania informacji dla nie tylko polskojęzycznych, ale również innych języków wspieranych przez ten model. Właśnie taki proces destylacji wiedzy zbadali w swojej pracy Piperno i in. \cite{multilingualDomainTranslation}, z powodzeniem korzystając z anglojęzycznych zbiorów słownictwa o tematyce medycznej do poprawienia osiągów modeli dla danych w językach hiszpańskim, niemieckim i francuskim. Przeprowadzony w ten sposób dodatkowy trening pozwoliłby na znaczące polepszenie całościowego rozwiązania opracowywanego w tej pracy, umożliwiając skorzystanie z większej ilości danych dostępnych w języku angielskim i przełożenia zdobytej przez model wiedzy na język polski dla zbioru ADMEDVoice \cite{ADMEDVoice}.


\section{Metodologia}
\subsection{Ocena jakości embeddingów}
W celu rzetelenej weryfikacji powyższej hipotezy dotyczącej semantyki w reprezentacjach latentnych modeli ASR, sformułowany został eksperyment wykorzystujący istniejące modele embeddingowe dla tekstu wspierające wiele języków. Wykorzystując istniejące zbiory danych zawierające zarówno audio wypowiedzianego zdania, jak i jego transkrypcję, wygenerowane zostały przykłady treningowe dla każdego zdania inspirowane metodą funkcji straty trójkowej (\textit{triplet loss}) \cite{tripletLoss}.
...
Na początku dla transkrypcji każdego zdania w zbiorze danych wygenerowane zostały jego embeddingi za pomocą każdego z ecenianych modeli embeddingowych. Następnie, dla każdego zdania (oznaczonego jako $x^a$ -- \textit{anchor}), wyliczono dystans (norma L2) do każdego z pozostałych zdań dla każdego modelu embeddingowego:

\begin{equation}
    D(x_{emb}^a, x_{emb}) = \| x_{emb}^a - x_{emb} \|_2^2 = \sqrt{(x_{emb}^a - x_{emb})^2}
\end{equation}

Następnie, na podstawie wygenerowanych w ten sposób zbiorów odległości, wybrano losowo przykłady pozytywne $x^p$ i negatywne $x^n$. Przykłady pozytywne stanowiły losowe zdanie spośród najbliższych (najbardziej podobnych) dziesięciu przykładów, z kolei przykłady negatywne spośród najmniej podobnych czterdziestu procent zdań. Losowość miała na celu zapewnić stosunkowo równomierny rozkład przykładów negatywnych spośród całego zbioru danych.

Posiadając trzy przykłady $x^a$, $x^n$ i $x^p$ dla każdego unikalnego zdania w zbiorze, następnym krokiem było wygenerowanie embeddingów dla każdego z ecenianych modeli ASR. Następnie, były one porównywane ze sobą za pomocą metryki $m(x_1, x_2)$, otrzymując w rezultacie ewaluacje relatywnego podobieństwa między $x^a$ i kolejno $x^p$ i $x^n$:
...
Metryką, wykorzystaną podczas tej ewaluacji to \textit{Maximum Mean Discrepancy} (MMD) \cite{mmd}. Jest ona przeznaczona do porównywania różnicy między dwoma dwuwymiarowymi tensorami, które stanowią w tym eksperymencie embeddingi wygenerowane przez modele ASR. Jest ona wyrażana wzorem:
\begin{equation}
...
Ze względu na określanie różnicy zamiast podobieństwa między dwoma reprezentacjami, w ramach porównania opisanego równaniem \eqref{eq:tripletDiff}, wartość ewaluacji $S(x_{emb, asr})$ będzie dodatnia, jeśli model poprawnie uchwycił relatywne podobieństwa semantyczne przykładów $x^a$, $x^n$ i $x^p$. W ten sposób możliwe jest ilościowe podejście do ewaluacji jakości w semantyczności reprezentacji modelu w ramach danego zbioru danych.

Ponadto, w celu zbadania rozkładu wartości ewaluacji $S(x_{emb, asr})$, dla zagregowanych w ramach modeli embeddingowych wyników dopasowany został rozkład Student-t, dla którego funkcja gęstości prawdopodobieństwa jest wyrażana wzorem:
...
Pozwala on na analizę statystyczną zachowania modeli w ramach proponowanego podejścia ilościowego, dostarczając informacji na temat stabilności i zdolności modelu do separacji przykładów pozytywnych i negatywnych, które są niezwykle cennymi czynnikami przy doborze właściwego modelu do dalszych eksperymentów.

\subsection{Analiza rozkładu embeddingów w przestrzeni}
...
gdzie $E \in \mathbb{R}^{N \times d}$ oznacza macierz embeddingów z $N$ próbkami o wymiarze $d$, a $\mu \in \mathbb{R}^{d}$ jest średnią embeddingów po poolingu na podstawie średniej. Macierz kowariancji $\frac{1}{N-1}(E-\mu)^\top(E-\mu)$ jest poddawana dekompozycji do postaci $U \Lambda U^\top$, gdzie $U$ zawiera wektory własne (ang. \textit{eigenvectors}), a $\Lambda$ odpowiadające im wartości własne (ang. \textit{eigenvalues}) (posortowane malejąco). Wybielone embeddingi $E_{\mathrm{wyb}}$ otrzymywane poprzez centrowanie, rotację oraz skalowanie każdej składowej głównej przez $(\lambda_i + \varepsilon)^{-1/2}$, gdzie $\varepsilon > 0$ zapewnia stabilność numeryczną (w tych eksperymentach $\varepsilon = 10^{-5}$).


Kolejną transformacją, w świetle której zbadano powyższe zjawisko, jest przekształcenie \textit{All But the Top} \cite{abtt}, które również używane jest w celu poprawienia końcowej jakości embeddingów generowanych przez modele. Polega ono na zgromadzeniu embeddingów wokół początku układu współrzędnych, usunięcia pierwszego komponentu na podstawie analizy PCA i normalizacji L2. ABTT pozwala na sprawdzenie, czy informacje o języku mają wpływ na rozkład danych w przestrzeni embeddingowej, podobnie jak w przypadku wybielania. Może być ono jednak interpretowane jako mniej destruktywne dla oryginalnego rozkładu embeddingów, ponieważ nie dekoreluje ono danych.

Proces transformacji danych metodą ABTT wyrażany jest wzorem \eqref{eq:abtt}:
...
\subsection{Ocena modeli ASR}
W ramach kompleksowej ewaluacji różnych modeli ASR wybrano pięć dostępnych modeli reprezentujących aktualny stan wiedzy (ang. \textit{state-of-the-art}) w dziedzinie transkrypcji audio. Modele te trenowane były w celu maksymalizacji dokładności transkrypcji, bez narzuconych ograniczeń dotyczącym maksymalizacji podobieństwa między reprezentacjami dla semantycznie podobnego sygnału audio, lecz ich osiągi dla transkrypcji dla wielu języków wskazują na częściowo semantyczne osadzenie ich reprezentacji.

Pierwszym z nich jest Whisper autorstwa OpenAI, wspierający ponad 90 języków \cite{whisper}. Jak wykazały badania Rao i in. \cite{whispaSemanticAlign}, wykazuje się on stosunkową łatwością do przekształcenia jego reprezentacji do reprezentacji semantycznej, co jest częściowym celem proponowanego systemu. Ponadto, został on wykorzystany z powodzeniem do ASR na zbiorze ADMEDVoice \cite{ADMEDVoice}, co wykazuje jego zdolność do radzenia sobie na specjalistycznych danych medycznych w języku polskim po odpowiednim dotrenowaniu.
...
Następnym z ecenianych modeli jest HuBERT autorstwa Mety \cite{hsu2021hubert}. Trenowany jest on na podstawie predykcji klas, oznaczonych jako identyfikator klastra wynikającego z klasteryzacji za pomocą algorytmu k-means cech ekstrapolowanych z sygnału audio za pomocą modelu Wav2Vec 2.0, stanowiąc swoiste ulepszenie metody Wav2Vec 2.0. Sam HuBERT nie wspiera języków poza językiem angielskim, lecz pokrewne modele z rodziny mHuBERT oferują wsparcie dla wielu innych języków, stąd też do ewaluacji tej architektury skorzystano dodatkowo z modelu mHuBERT-147 \cite{mhubert}.
...
Każdy z tych modeli charakteryzuje się różnym rodzajem treningu, co oprócz rozmiaru danych treningowych, stanowić może najbardziej różnicujący je czynnik. Whisper trenowny był stricte w celu generowania transkrypcji wejściowego sygnału mowy -- dla tego modelu w tej pracy wykoszystano wyłącznie jego enkoder. Wav2Vec, Data2Vec i HuBERT uczone były ekstarkcji cech z przetwarzanych danych, korzystając z różnych metod i funkcji straty, po czym osobnemu treningowi poddawany był blok odpowiadający za zadanie transkrypcji. W dalszych eksperymentach wykorzystano pełne modele, nie zawierające tej dodatkowej części w celu uzyskania embeddingów. SeamlessM4T stanowi z kolei pełnoprawny model multimodalny, wykorzystujący zarówno dane tekstowe i audio w ramach treningu, którego celem było ujednolicenie reprezentacji dla różnych modalnośći. W ramach jego ewaluacji wykorzystany został wyłącznie jego enkoder mowy.

\subsection{Ocena modeli embeddingowych}
W celu przeprowadzenia rzetelnej analizy możliwości modeli ASR w świetle enkodowania informacji semantycznych, do eksperymentów wykorzystano trzy różne modele embeddingowe.
...
Drugim i jednocześnie najstarszym z nich był model wielojęzykowy SBERT \cite{reimers2019sentence} wykorzystujący enkoder XLM-R \cite{conneau2020unsupervised} dotrenowany na zbiorze STSB \cite{stsb}. Enkoder XLM-R wytrenowany został w celu kodowania reprezentacji tekstu agnostycznych językowo, dzięki czemu reprezentacje latentne są jak najbardziej niezależne pod względem języka. Następnie model SBERT wykorzystuje sieć syjamską, porównując bezpośrednio dwie generowane reprezentacje, redukując dystans pomiędzy semantycznie podobnymi tekstami, uzyskując znaczne przyspieszenie treningu w porównaniu z bazowym modelem BERT.
...
W ramach eksperymentu poddano ewaluacji działanie powyższych modeli dla dwóch zbiorów danych. Pierwszym z nich był anglojęzyczny zbiór danych \textit{Medical Speech, Transcription, and Intent} \cite{generalMedicalDataset}. Zawiera on 8,5 godzin audio wraz z transkrypcjami dotyczącymi powszechnych objawów chorób, takich jak gorączka i ból gardła. Został on wybrany ze względu na połączenie tematyki powiązanej z docelowym zbiorem danych dla proponowanych w tej pracy modeli, jak i brakiem ściśle specjalistycznego języka medycznego, w ramach którego modele ASR i embeddingowe powinny poradzić sobie dobrze bez dodatkowego treningu. Drugim wybranym zbiorem był zbiór FLEURS \cite{fleurs}, zawierający szereg zdań wypowiedzianych w 102 językach. Wykorzystano podzbiory zdań dla języka polskiego i angielskiego, każdy mający około 12 godzin nagrań wraz z ich transkrypcjami, w celu ewaluacji modeli w ramach ich dokładności dla wielu języków.

W ramach obliczania metryki MMD \eqref{eq:MMD}, skorzystano z liniowej funkcji jądra $k(x_1, x_2)$:
\begin{equation}
    k_{\text{lin}}(x, y) = x^{\top} y
\end{equation}

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/semantics/labse_output.png}
    \caption{Rozkład przykładów pozytywnych i negatywnych dla modelu LaBSE w ramach zbioru \textit{Medical Speech, Transcription, and Intent}. Kolorem pomarańczowym oznaczono liczbę zdań, dla których dany przykład jest przykładem negatywnym, niebieskim -- pozytywnym}
...
\begin{figure}[!ht]
    ...
    \caption{Rozkład przykładów pozytywnych i negatywnych dla modelu LaBSE w ramach zbioru FLEURS. Kolorem pomarańczowym oznaczono liczbę zdań, dla których dany przykład jest przykładem negatywnym, niebieskim -- pozytywnym}
...
Eksperymenty zostały przeprowadzone z wykorzystaniem języka Python w wersji 3.12. Modele ASR i embeddingowe wraz z ich wagami zostały pobrane ze strony HuggingFace za pomocą bibliotek \textit{transformers} i \textit{sentence-transformers}. Do szybkiego obliczenia odległości między embeddingami wraz z wyłonieniem najbliższych i najdalszych embeddingów wykorzystano biblioteki \textit{Pytorch} i \textit{keOps}. Do obliczeń pomocniczych wykorzystano bibliotekę \textit{numpy}, natomiast do wizualicji -- \textit{matplotlib}. Eksperyment przeprowadzony został na serwerze obliczeniowym przy użyciu karty graficznej Nvidia RTX A5000.
\subsection{Analiza wyników}

\subsubsection{Analiza enkodowania znaczenia semantycznego mowy}

Rozkłady wartości metryki $S(x_{emb,asr})$ \eqref{eq:tripletDiff} dla poszczególnych modeli ASR przedstawione są na wykresach zamieszczonych w dodatku~\ref{app:semantics}.

\begin{table}[!ht]
\centering
\caption{$S(x_{emb,asr}) > 0$ dla zbioru \textit{Medical Speech, Transcription, and Intent} (liczba przykładów = 5327)}
...
% Cone
%
\subsubsection{Analiza efektu stożka dla wybranych wielojęzycznych modeli ASR}
...
Oprócz modeli poddanych analizie w ramach badania efektu stożka, tj. Whisper, mHuBERT i SeamlessM4T, w tym eksperymencie uwzględniono również bazowy model HuBERT, który wykazywał dobre wyniki w ramach zbiorów \textit{Medical Speech, Transcription, and Intent} i \textit{Fleurs}.
