...
W modelu boolowskim wynik wyszukiwania ma charakter binarny i nie uwzględnia stopnia dopasowania dokumentu do intencji użytkownika. Rozwój systemów typu \textit{free text retrieval} wprowadził miary takie jak TF-IDF (ang. \textit{Term Frequency-Inverse Document Frequency}), kwantyfikujące istotność terminów w kolekcji dokumentów \cite{sparckjones1972statistical, salton1988term}. Współczynnik TF-IDF przypisuje wyższą wagę terminom występującym często w danym dokumencie, ale rzadko w całej kolekcji. Pozwala to zidentyfikować słowa kluczowe charakterystyczne dla treści dokumentu.
...
Faza wyszukiwania inicjowana jest przez zapytanie użytkownika, które jest transformowane do tej samej przestrzeni wektorowej co zindeksowane dokumenty. Na podstawie miary podobieństwa (typowo podobieństwa kosinusowego lub odległości euklidesowej) system identyfikuje $k$ najbardziej podobnych fragmentów dokumentów. W fazie generacji model językowy otrzymuje kontekst składający się z wyszukanych fragmentów dokumentów oraz oryginalnego zapytania użytkownika. Na tej podstawie generuje odpowiedź, która może być weryfikowana przez użytkownika poprzez porównanie z dostarczonymi źródłami.
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/rag}}
...
Architektura Transformer \cite{vaswani2017attention}, oparta wyłącznie na mechanizmie uwagi bez komponentów rekurencyjnych, znalazła szerokie zastosowanie w modelach ASR nowej generacji. Modele te, określane jako end-to-end, eliminują tradycyjny pipeline przetwarzania składający się z oddzielnych modułów akustycznego, leksykalnego i językowego. Zamiast tego realizują bezpośrednie mapowanie surowego sygnału audio na sekwencję tekstową. Upraszcza to architekturę systemu i usuwa propagację błędów między modułami wewnątrz samego systemu ASR. Należy odróżnić tę kaskadę od kaskady rozważanej w niniejszej pracy, w której błąd transkrypcji propaguje się dalej, do etapu embeddingu i wyszukiwania.
...
Proces uczenia samonadzorowanego w modelu Wav2Vec 2.0 opiera się na maskowaniu fragmentów sekwencji wejściowej oraz uczeniu kontrastywnym (ang. \textit{contrastive learning}). Model jest trenowany do identyfikacji prawidłowych kwantyzowanych reprezentacji zamaskowanych segmentów ze zbioru negatywnych przykładów. Podejście to pozwala na wykorzystanie ogromnych ilości nieetykietowanych nagrań audio do uczenia reprezentacji akustycznych przydatnych w zadaniach rozpoznawania mowy, bez konieczności korzystania z dużych ilości transkrybowanych nagrań.
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/wav2vec}}
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/hubert}}
...
W przeciwieństwie do modelu Wav2Vec 2.0, Whisper operuje na spektrogramach w skali melowej. Stanowią one reprezentację częstotliwościową sygnału audio uwzględniającą charakterystykę ludzkiego słuchu. Takie podejście umożliwia normalizację dynamiki i redukcję szumu na etapie ekstrakcji cech. Jednocześnie przenosi ciężar modelowania z niskopoziomowych cech akustycznych na wyższe zależności językowe i kontekstowe. Enkoder przetwarza melspektrogram i generuje sekwencję reprezentacji ukrytych. Dekoder, działający w sposób autoregresyjny, generuje kolejne tokeny tekstu na podstawie reprezentacji z enkodera oraz wcześniej wygenerowanych tokenów. Schemat architektury przedstawiono na rysunku~\ref{fig:whisperArchitecture}.
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/whisper}}
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/data2vec}}
...
Otwartym pytaniem pozostaje, czy reprezentacje ukryte generowane przez modele ASR zawierają informację semantyczną wykraczającą poza poziom akustyczny i fonetyczny. Zagadnienie to stanowi motywację dla badań nad bezpośrednim wykorzystaniem reprezentacji modeli ASR w zadaniach wyszukiwania informacji, a jego weryfikacji poświęcono rozdział~\ref{sec:latent}.
...
Pojęcie semantycznej reprezentacji wywodzi się z hipotezy dystrybucyjnej Harrisa i modeli embeddingowych omówionych w podrozdziale~\ref{sec:embeddingi}, w których relacje semantyczne odwzorowywane są przez zależności geometryczne między wektorami.
...
Analiza geometryczna opiera się na założeniu, że rozkład reprezentacji w przestrzeni wielowymiarowej odzwierciedla jakość zakodowanej w nich informacji. Ethayarajh wykazał, że reprezentacje kontekstowe modeli BERT, ELMo i GPT-2 zajmują wąski stożek przestrzeni \cite{cone}. W efekcie podobieństwo kosinusowe dowolnych dwóch losowych reprezentacji przyjmuje wartości znacząco dodatnie. Zjawisko to, określane jako anizotropia przestrzeni, ogranicza zdolność miar podobieństwa do rozróżniania przykładów. Gao i in. wskazali, że degeneracja reprezentacji stanowi systematyczną konsekwencję treningu modeli generatywnych z funkcją straty opartą na wiarygodności \cite{gao2019representation}.
...
Metody probingowe (ang. \textit{probing}) opierają się na prostym rozumowaniu: jeżeli w reprezentacji wytworzonej przez daną warstwę modelu zakodowana jest pewna informacja, powinna ona dać się z niej odczytać za pomocą nieskomplikowanego klasyfikatora. Skuteczność takiego klasyfikatora, najczęściej liniowego, staje się wówczas miarą tego, na ile badana własność jest w reprezentacji dostępna \cite{alain2017probes}. Conneau i in. przenieśli tę ideę na poziom całych zdań \cite{conneau2018cram}. Opracowany przez nich zestaw zadań sprawdza, które własności lingwistyczne, od długości zdania po głębokość drzewa składniowego, zachowują embeddingi zdaniowe. Belinkov i Glass zwracają jednak uwagę na zasadnicze ograniczenie tego podejścia \cite{belinkov2019analysis}. Powodzenie klasyfikatora probingowego świadczy wyłącznie o tym, że informacja w reprezentacji się znajduje, nie zaś o tym, że model rzeczywiście się nią posługuje.

W domenie mowy najistotniejszym przykładem analizy probingowej jest praca Pasada i in., w której zbadano kolejne warstwy modelu Wav2Vec 2.0 \cite{pasad2021layer}. Wykazano, że model wykazuje charakterystykę zbliżoną do autoenkodera, przy czym warstwy środkowe kodują informację fonetyczną i leksykalną w stopniu najwyższym, natomiast warstwy końcowe wracają w kierunku reprezentacji zbliżonych do wejściowych. Obserwacja ta ma bezpośrednie znaczenie dla doboru warstwy wykorzystywanej jako źródło reprezentacji w architekturze proponowanej w niniejszej pracy. Pokrewny kierunek badań zakłada wymuszenie interpretowalności już na poziomie architektury, poprzez dyskretyzację reprezentacji do skończonego inwentarza jednostek, który można zestawiać z kategoriami fonetycznymi. Zhou i in. porównali stosowane w tym celu warianty kwantyzacji wektorowej, analizując ich wpływ na jakość reprezentacji mowy \cite{Zhou2020ACO}.

Podejścia jakościowe koncentrują się na rozkładzie reprezentacji na komponenty poddające się interpretacji. Punktem wyjścia jest obserwacja, że pojedyncze neurony sieci są zazwyczaj polisemantyczne, to znaczy aktywują się w odpowiedzi na wiele niepowiązanych ze sobą zjawisk. Wynika to z superpozycji cech w przestrzeni o ograniczonej wymiarowości. Bricken i in. wykazali, że rzadkie autoenkodery (ang. \textit{sparse autoencoders}, SAE) trenowane na aktywacjach modelu językowego pozwalają na rozłożenie ich na słownik cech znacznie bliższych monosemantyczności \cite{bricken2023monosemanticity}. Wyniki te zostały potwierdzone i rozszerzone w pracy Hubena i in., w której wykazano wyższą interpretowalność cech uzyskanych metodą SAE w porównaniu z alternatywnymi metodami dekompozycji \cite{huben2024sparse}.

Podejście to znajduje coraz szersze zastosowanie w domenie audio. Paek i in. zastosowali rzadkie autoenkodery do przestrzeni cech ukrytych modeli audio, wyodrębniając składowe odpowiadające interpretowalnym zjawiskom akustycznym i muzycznym \cite{paek2025learninginterpretablefeaturesaudio}. Varshavsky-Hassid i in. wykazali natomiast obecność semantycznie nacechowanych kierunków w przestrzeni cech ukrytych dyfuzyjnych modeli syntezy mowy \cite{varshavskyhassid2024semanticlatentspacediffusionbased}. Prace te wskazują, że przestrzenie cech ukrytych modeli przetwarzających mowę zawierają strukturę wykraczającą poza poziom akustyczny, co stanowi bezpośrednią motywację dla badań prowadzonych w niniejszej pracy.

\subsection{Wyjaśnialność w systemach wyszukiwania opartych na mowie}

Jak wskazano w podrozdziale~\ref{sec:s2r}, pominięcie transkrypcji odbiera użytkownikowi możliwość kontroli działania systemu. Analiza reprezentacji stanowi częściową rekompensatę tego ograniczenia. Pozwala ona na uzasadniony dobór modelu ASR i warstwy stanowiącej źródło reprezentacji, a także na diagnostykę własności przestrzeni wejściowej przed treningiem warstwy projekcji. Zestawienie metod omówionych w tej sekcji przedstawiono w tabeli~\ref{tab:xaiMetody}.
...
\bottomrule
\end{tabular}
\caption{Metody analizy reprezentacji wykorzystywane do oceny ich semantyczności. Skrót MMD oznacza \textit{Maximum Mean Discrepancy}, ABTT -- \textit{All-but-the-Top}, a SVCCA -- \textit{Singular Vector Canonical Correlation Analysis}.}
\label{tab:xaiMetody}
\end{table}
...
Rosnące zapotrzebowanie na interfejsy głosowe motywuje rozwój systemów wyszukiwania informacji zdolnych do przetwarzania zapytań audio. W odpowiedzi na ograniczenia architektury kaskadowej, omówione w rozdziale~\ref{sec:wprowadzenie}, powstały dwa główne nurty badawcze. Pierwszy koncentruje się na architekturach end-to-end, które uczą się bezpośredniego mapowania sygnału audio na reprezentacje semantyczne, omijając etap transkrypcji. Drugi wykorzystuje podejścia multimodalne, tworzące wspólną przestrzeń reprezentacji dla danych audio, tekstu i obrazu.

\subsection{Podejścia end-to-end}

\textbf{SpeechDPR.} Model SpeechDPR (ang. \textit{Speech Dense Passage Retriever}) \cite{lin2024speechdprendtoendspokenpassage} stanowi pierwsze rozwiązanie typu end-to-end dla problemu wyszukiwania fragmentów mowy w zadaniach odpowiadania na pytania. Model uczy się semantycznych reprezentacji na poziomie zdaniowym poprzez destylację wiedzy z systemu kaskadowego składającego się z nienadzorowanego ASR oraz tekstowego retrievera opartego na gęstych reprezentacjach. Proces ten nie wymaga ręcznie transkrybowanych danych mowy, co stanowi istotną zaletę w kontekście języków o ograniczonych zasobach. Schemat procesu treningu przedstawiono na rysunku~\ref{fig:speechdprArchitecture}.

\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/speechdpr}}
...
\textbf{SpeechRAG.} SpeechRAG \cite{min2025speechretrievalaugmentedgenerationautomatic} rozszerza architekturę RAG o bezpośrednie przetwarzanie audio. System składa się z retrievera audio oraz generatora opartego na modelu języka mowy (SLM, ang. \textit{Speech Language Model}). Kluczową innowacją jest dostrojenie pretrenowanego enkodera mowy do roli adaptera podłączonego do zamrożonego retrievera tekstowego. Dzięki wyrównaniu przestrzeni embeddingów retriever audio może wyszukiwać fragmenty na podstawie zapytań tekstowych. W fazie generacji model SLM otrzymuje jako kontekst oryginalne fragmenty audio, eliminując straty informacyjne wynikające z konwersji mowa-tekst. Architekturę systemu przedstawiono na rysunku~\ref{fig:speechragArchitecture}.

\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/speechrag}}
...
\textbf{SONAR.} SONAR \cite{sonar} definiuje przestrzeń reprezentacji zdaniowych o stałej długości, wspólną dla dwustu języków tekstu oraz dla mowy. Przestrzeń tę wyznacza enkoder tekstu trenowany zadaniem tłumaczenia w układzie enkoder-dekoder, a nie funkcją kontrastywną. Enkodery mowy dołączane są do niej dopiero później, metodą teacher-student, w której wzorcem jest reprezentacja tekstowa transkrypcji tego samego zdania. Nagranie i jego tłumaczenie na dowolny obsługiwany język trafiają dzięki temu w to samo miejsce przestrzeni. Pozwala to wyszukiwać dokumenty tekstowe wprost nagraniem, bez etapu transkrypcji. Sposób budowy tej przestrzeni przedstawiono na rysunku~\ref{fig:sonarArchitecture}. Reprezentacje te posłużyły do wydobywania równoległych par mowa-tekst z korpusów nierównoległych, wykorzystanych przy budowie modelu SeamlessM4T \cite{m4t}. Dla niniejszej pracy SONAR jest najbliższym punktem odniesienia. Pokazuje, że dostrojenie enkodera mowy do gotowej, zamrożonej przestrzeni tekstu wystarcza do wyszukiwania międzymodalnego. Zarazem wyznacza granicę takiego rozwiązania: ścieżka głosowa nie wyszuka lepiej niż enkoder tekstu, do którego ją dopasowano.

\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/sonar}}
    \caption{Budowa przestrzeni SONAR. Przestrzeń wyznacza enkoder tekstu uczony zadaniem
    tłumaczenia przez wektor zdania o stałej długości; enkoder mowy dołączany jest do niej
    osobno, przez dopasowanie wektora nagrania do wektora transkrypcji tego samego zdania przy
    zamrożonym enkoderze tekstu. Obie modalności trafiają dzięki temu do jednej przestrzeni.}
    \label{fig:sonarArchitecture}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/wavrag}}
...
\textbf{Wyszukiwanie multimodalne dla ASR.} Kolehmainen i in. \cite{kolehmainen2024multimodalretrievallargelanguage} zbadali zastosowanie wyszukiwania multimodalnego do poprawy jakości rozpoznawania mowy. Zaproponowany system wykorzystuje dwie techniki: kNN-LM (ang. k-Nearest Neighbors Language Model, model językowy oparty na algorytmie $k$-najbliższych sąsiadów) oraz mechanizmy uwagi krzyżowej (ang. cross-attention). Wyszukiwanie oparte na mowie prowadzi do redukcji WER o nawet 50\% w porównaniu z bazowym modelem. System osiągnął wyniki zgodne ze stanem wiedzy (ang. state-of-the-art) w zadaniu rozpoznawania mowy na zbiorze Spoken-SQuAD. Sposób włączenia wyszukanych fragmentów do procesu rozpoznawania przedstawiono na rysunku~\ref{fig:multimodalAsrArchitecture}.

\begin{figure}[htbp]
    \centering
    \adjustbox{max width=\linewidth}{\input{tikz/multimodalasr}}
...
Przedstawione podejścia łączy dążenie do eliminacji lub ograniczenia roli tradycyjnego ASR jako pośredniego etapu przetwarzania. Zarówno architektury end-to-end, jak i multimodalne wykazują przewagę nad systemami kaskadowymi w warunkach niskiej jakości rozpoznawania mowy. Żadna z omówionych prac nie wykazuje jednak, by informacja prozodyczna, zachowana dzięki pominięciu transkrypcji, była w tych systemach faktycznie wykorzystywana przy wyszukiwaniu. Wyznacza to kierunek rozwoju systemów wyszukiwania informacji opartych na danych mowy.
